...
This part of the documentation describes options and concepts for <code>mir-seek <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking mir-seek pipeline output directory is fast and easy! In its most basic form, <code>mir-seek <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./mir-seek unlock [-h] --output OUTPUT
```
...
mir-seek unlock --output /data/$USER/output
```