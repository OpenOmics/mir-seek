...
def setup(sub_args, ifiles, repo_path, output_path):
    ...
    if sub_args.genome.endswith('.json'):
        # Provided a custom reference genome JSON file
    ...
    if sub_args.genome.endswith('.json'):
        # Check if more than one reference genome
        # exists in the custom JSON file, we do 
        # not support this!
        if len(config['references'].keys()) >= 2:
            err('Error: Custom reference JSON file contains more than one reference genome: {}.'.format(
                    list(config['references'].keys())
                )
            )
            fatal('  └── Please remove one of the references genomes from {0} and try again!'.format(
                    sub_args.genome 
                )
            )
